 ...
 *	 or <textarea> depending on the QCKeditorTextMode (see below).
 *
 ...
 *	@property QCKeditorTextMode TextMode plain text <textarea> or WYSIWYG.
 ...
 *	@package Controls
 */
	class QCKeditor extends QControl {
		///////////////////////////
		...
		protected $strTextMode = QCKeditorTextMode::XHTML;
		protected $strCrossScripting = QCrossScripting::Deny;

		// TEXTAREA - when QCKeditorTextMode::Plain
        ...
        public function __construct($objParentObject, $strControlId = null) {
            ...
            $this->strBasePath = __PLUGIN_ASSETS__ . "/QCKEditor/js/";
			$this->CustomConfigurationsPath = __PLUGIN_ASSETS__ . "/QCKEditor/js/config.js";
        }
		...
		public function GetAttributes($blnIncludeCustom = true, $blnIncludeAction = true) {
			...
			if ($this->strTextMode == QCKeditorTextMode::Plain) {
				if (!$this->blnWrap)
		...
		protected function GetControlHtml() {
			if($this->strTextMode == QCKeditorTextMode::XHTML){
				require_once(__DOCROOT__. __PLUGIN_ASSETS__ .'/QFCKEditor/js/fckeditor.php');
	...
	abstract class QCKeditorTextMode {
		const XHTML = 'XHTML';
		const Plain = 'Plain';
	}
?>
